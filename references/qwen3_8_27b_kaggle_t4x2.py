# -*- coding: utf-8 -*-
"""Qwen3.8-27B QLoRA on a Kaggle T4 x2 kernel (Unsloth notebook export).

Supplied on 2026-09-12 as a py-percent export of Unsloth's Qwen3.8-27B Kaggle
notebook. Kept verbatim as a research reference, with one edit: the install
cell's IPython magics are commented out in the way Colab's own .py exports do,
so the file parses as Python. The notebook is licensed LGPL-3.0 by its authors
(see its footer).

Read docs/model-and-hardware.md ("Second lane: Kaggle T4 x2") for what this
run proves and what it cannot. It is a 4-bit, 1,024-token plumbing path, not
the capability path this project trains on.
"""
